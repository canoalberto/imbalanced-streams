...
import moa.classifiers.trees.HoeffdingAdaptiveTree.NewNode;
import moa.classifiers.trees.HoeffdingTreeHistogram.FoundNode;
import moa.classifiers.trees.HoeffdingTreeHistogram.Node;
import moa.classifiers.trees.HoeffdingTreeHistogram.SplitNode;
 ...
 * @version $Revision: 7 $
 */
public class HoeffdingTreeHistogram extends AbstractClassifier implements MultiClassClassifier,
                                                                 CapabilitiesHandler {
    ...
    public ClassOption numericEstimatorOption = new ClassOption("numericEstimator",
            ...
            "GaussianNumericAttributeClassObserverHistogram");
    ...
    public ClassOption nominalEstimatorOption = new ClassOption("nominalEstimator",
            ...
            "NominalAttributeClassObserverHistogram");
    ...
    public static class Node extends AbstractMOAObject {
        ...
        //weight of instances observed from the beginning (inherited from father)
        protected DoubleVector observedClassDistribution;
        
        //# instances observed from the beginning (inherited from father)
        protected DoubleVector observedClassDistributionNode;
        
        //# instances observed from the creation to the node (not inherited from father) 
        //corresponds to the values saved in attributeObservers
        protected DoubleVector observedSinceCreation = new DoubleVector();

        public Node(double[] classObservations, double[] classObservationsNode, double[] classObservationsSinceCreation) {
            this.observedClassDistribution = new DoubleVector(classObservations);
            this.observedClassDistributionNode = new DoubleVector(classObservationsNode);
            this.observedSinceCreation = new DoubleVector(classObservationsSinceCreation);
        }
        ...
        public double[] getObservedClassDistributionNode() {
            return this.observedClassDistributionNode.getArrayCopy();
        }
        
        public double[] getObservedWhenCreated() {
        	return this.observedSinceCreation.getArrayCopy();
        }                
        ...
        public void describeSubtree(HoeffdingTreeHistogram ht, StringBuilder out,
                int indent) {
    ...
    public static class SplitNode extends Node {
        ...
        public SplitNode(InstanceConditionalTest splitTest,
            ...
            super(classObservations,classObservationsNode,classObservationsSinceCreation);
    ...
    public static abstract class LearningNode extends Node {
        ...
        public LearningNode(double[] initialClassObservations, double[] initialClassObservationsNode, double[] classObservationsSinceCreation) {
            super(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        }

        public abstract void learnFromInstance(Instance inst, HoeffdingTreeHistogram ht);
    }
    ...
    public static class InactiveLearningNode extends LearningNode {
        ...
        public InactiveLearningNode(double[] initialClassObservations, double[] initialClassObservationsNode,double[] classObservationsSinceCreation) {
            super(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        }

        @Override
        public void learnFromInstance(Instance inst, HoeffdingTreeHistogram ht) {
    ...
    public static class ActiveLearningNode extends LearningNode {
        ...
        public ActiveLearningNode(double[] initialClassObservations, double[] initialClassObservationsNode,double[] classObservationsSinceCreation) {
            super(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        ...
        public AttributeSplitSuggestionHistrogram[] getBestSplitSuggestions(
                SplitCriterion criterion, HoeffdingTreeHistogram ht) {
            List<AttributeSplitSuggestionHistrogram> bestSuggestions = new LinkedList<AttributeSplitSuggestionHistrogram>();
            double[] preSplitDist = this.observedClassDistribution.getArrayCopy();            
            if (!ht.noPrePruneOption.isSet()) {
                // add null split as an option
                bestSuggestions.add(new AttributeSplitSuggestionHistrogram(null,
                        new double[0][],new double[0][], criterion.getMeritOfSplit(
                        preSplitDist,
            ...
            for (int i = 0; i < this.attributeObservers.size(); i++) {
                AttributeClassObserver obs = this.attributeObservers.get(i);
                if (obs != null) {
                	AttributeSplitSuggestionHistrogram bestSuggestion = obs.getBestEvaluatedSplitSuggestionHistogram(criterion,
            ...
            return bestSuggestions.toArray(new AttributeSplitSuggestionHistrogram[bestSuggestions.size()]);
        }
    ...
    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
            double[] classObservations, double[] classObservationsNode, double[] classObservationsSinceCreation, int size) {
        return new SplitNode(splitTest, classObservations, classObservationsNode, classObservationsSinceCreation, size);
    }
    
    protected SplitNode newSplitNode(InstanceConditionalTest splitTest,
            double[] classObservations, double[] classObservationsNode,double[] classObservationsSinceCreation) {
        return new SplitNode(splitTest, classObservations, classObservationsNode,classObservationsSinceCreation);
    }
    ...
    protected void attemptToSplit(ActiveLearningNode node, SplitNode parent,
        ...
        if (!node.observedClassDistributionIsPure()) {
            ...
            AttributeSplitSuggestionHistrogram[] bestSplitSuggestions = node.getBestSplitSuggestions(splitCriterion, this);
            ...
            if (bestSplitSuggestions.length < 2) {
                ...
                AttributeSplitSuggestionHistrogram bestSuggestion = bestSplitSuggestions[bestSplitSuggestions.length - 1];
                AttributeSplitSuggestionHistrogram secondBestSuggestion = bestSplitSuggestions[bestSplitSuggestions.length - 2];
            ...
            if (shouldSplit) {
            	AttributeSplitSuggestionHistrogram splitDecision = bestSplitSuggestions[bestSplitSuggestions.length - 1];
                if (splitDecision.splitTest == null) {
                    // preprune - null wins
                    deactivateLearningNode(node, parent, parentIndex);
                } else {
                    SplitNode newSplit = newSplitNode(splitDecision.splitTest,node.getObservedClassDistribution(),
                    		node.getObservedClassDistributionNode(),node.getObservedWhenCreated(),splitDecision.numSplits());
                    for (int i = 0; i < splitDecision.numSplits(); i++) {
                        Node newChild = newLearningNode(splitDecision.resultingClassDistributionFromSplit(i),
                        		splitDecision.resultingClassDistributionFromSplitNode(i), new double[0]);
                        //newChild.observedSinceCreation = new DoubleVector();
    ...
    protected void deactivateLearningNode(ActiveLearningNode toDeactivate,
        ...
        Node newLeaf = new InactiveLearningNode(toDeactivate.getObservedClassDistribution(), toDeactivate.getObservedClassDistributionNode(), toDeactivate.getObservedWhenCreated());
        if (parent == null) {
    ...
    protected void activateLearningNode(InactiveLearningNode toActivate,
            SplitNode parent, int parentBranch) {
        Node newLeaf = newLearningNode(toActivate.getObservedClassDistribution(), toActivate.getObservedClassDistributionNode(), toActivate.getObservedWhenCreated());
        if (parent == null) {
            this.treeRoot = newLeaf;
        } else {
            parent.setChild(parentBranch, newLeaf);
    ...
    public static class LearningNodeNB extends ActiveLearningNode {
        ...
        public LearningNodeNB(double[] initialClassObservations, double[] initialClassObservationsNode,double[] classObservationsSinceCreation) {
            super(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        }
    ...
    public static class LearningNodeNBAdaptive extends LearningNodeNB {
        ...
        public LearningNodeNBAdaptive(double[] initialClassObservations,double[] initialClassObservationsNode,double[] classObservationsSinceCreation) {
            super(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        }
    ...
    protected LearningNode newLearningNode() {
        return newLearningNode(new double[0],new double[0], new double[0]);
    }
    ...
    protected LearningNode newLearningNode(double[] initialClassObservations,double[] initialClassObservationsNode, double[] classObservationsSinceCreation) {
        ...
        if (predictionOption == 0) { //MC
            ret = new ActiveLearningNode(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        } else if (predictionOption == 1) { //NB
            ret = new LearningNodeNB(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        } else { //NBAdaptive
            ret = new LearningNodeNBAdaptive(initialClassObservations,initialClassObservationsNode,classObservationsSinceCreation);
        }
        return ret;
    }
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
      if (this.getClass() == HoeffdingTreeHistogram.class)
