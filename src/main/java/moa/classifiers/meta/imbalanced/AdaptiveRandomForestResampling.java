/*
 *    AdaptiveRandomForestResampling.java
 * 
...
/**
 * Adaptive Random Forest with resampling
 *
 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestResampling extends AbstractClassifier implements MultiClassClassifier,
                                                                        CapabilitiesHandler {
    ...
    public void resetLearningImpl() {
        ...
        if(numberOfJobs != AdaptiveRandomForestResampling.SINGLE_THREAD && numberOfJobs != 1)
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        numberInstances[(int)instance.classValue()]++;
        ...
        for (int i = 0 ; i < this.ensemble.length ; i++) {
            ...
            double k = MiscUtils.poisson(this.lambdaOption.getValue(), this.classifierRandom) * (100.0 - ((numberInstances[(int)instance.classValue()] * 100.0) / instancesSeen)) / 100.0;
            if (k > 0) {
    ...
    protected void initEnsemble(Instance instance) {
        ...
        this.numberInstances = new int[instance.numClasses()];
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch(this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForestResampling.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForestResampling.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForestResampling.FEATURES_PERCENT:
        ...
        //  AdaptiveRandomForestResampling.FEATURES_M then nothing is performed in the
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == AdaptiveRandomForestResampling.class)
